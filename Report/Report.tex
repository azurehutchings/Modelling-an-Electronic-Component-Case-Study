...
\\Using a technique called RCM ordering, the empty space between the outer and inner diagonals can be greatly reduced. The b vector is also reordered using the same reordering vector to keep the equation $Ax=b$ equivalent. The reordered A matrix using RCM ordering and the banded matrix B will look like\\
\hspace{-8mm}
\begin{figure}
	\hspace{-8mm}
