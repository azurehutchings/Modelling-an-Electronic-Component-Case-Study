...
\begin{figure}[H]
	...
	\end{subfigure}
\end{figure}
The next step is to convert the RCM ordering to a banded storage matrix, denoted A$_{\text{band}}$. Because $A$ is symmetric, we can get away with only storing the upper or lower part of the matrix. The dimensions of A$_{\text{band}}$ are $\mathbb{R}^{n\times p}$, where n is the row or column count of $A$, and p is the upper bandwidth of $A$. To change the storage, we must create a new mapping from the elements of the upper portion of $A$ to A$_{\text{band}}$.
\begin{align*}
A(i,j)\to A_{\text{band}}(i,j-i+1), \text{ where } i,j < \text{dim}(A).
\end{align*}
