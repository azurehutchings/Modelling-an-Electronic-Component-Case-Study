...
\[x^{k+1}=Tx^k+c\]
where $T$ is the iteration matrix. This iteration matrix depends on which method is used.
\\\\
...
\begin{figure}[H]
	...
	\label{fig:res}
\end{figure}
In Figure \ref{fig:res} it can be seen that the Conjugate method has the least iterations. However, for the first few iterations the conjugate gradient got less accurate over the course of an iteration.

Your SOR takes longer than Gauss-Seidel to run, which does not make much sense. I assume you also timed the calculation of optimal omega in your SOR, and this is why, but this makes for an unfair comparison.
FLOPs in conjugate gradient method are severely underestimated, due to counting vector-vector multiplications as one FLOP (it is actually 2n-1).
The omega plot is satisfactory.
