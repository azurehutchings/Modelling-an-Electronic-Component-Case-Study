...
\begin{center}
\[\big{(}\frac{\partial T}{\partial x},\frac{\partial T}{\partial y}\big{)}\cdot (-1,0)^T = 0\]
...
\begin{center}
  \[\big{(}\frac{\partial T}{\partial x},\frac{\partial T}{\partial y}\big{)}\cdot \frac{1}{\sqrt{2}}(-1,-1)^T = 0\]
  \[\big{(}\frac{\partial T}{\partial x},\frac{\partial T}{\partial y}\big{)}\cdot (-1,-1)^T = 0\]
  \[\big{(}\frac{\partial T}{\partial x}+\frac{\partial T}{\partial y}\big{)} = 0\]
...
\begin{center}
\[\big{(}\frac{\partial T}{\partial x},\frac{\partial T}{\partial y}\big{)}\cdot (0,-1)^T = 0\]
...
There are three nodes that we care about on the boundary B-C. These are nodes (3,5), (4,4) and (5,3). Each of these nodes will have a different normal vector and so must all be calculated individually.
\paragraph*{Node (3,5):} At this node, the normal vector is going to be facing directly east. Even though the boundary's normal looks like it would be facing an angle above due east, it just faces directly east 
