...
The third iterative method that is used to solve for $x$ using CSR storage is the Successive Over-relaxation (SOR) method. This method is very similar to the Gauss-Seidel method.
\[b=Ax\]
\[0=Ax-b\]
\[0=\bigg(D\frac{\omega}{\omega}+L+U\bigg)x-b\]
\[0=\bigg(\frac{D-D(1-\omega)}{\omega}+L\bigg)x+Ux-b\]
\[-\bigg(\frac{D}{\omega}+L\bigg)x=\bigg(D\frac{1-\omega}{\omega}+U\bigg)x+b\]
\[x=\bigg(\frac{D}{\omega}+L\bigg)^{-1}\bigg(D\frac{1-\omega}{\omega}+U\bigg)x+\bigg(\frac{D}{\omega}+L\bigg)^{-1}b\]
The SOR method has much faster convergence than Jacobi and Gauss-Seidel method, provided an optimal value for $\omega$ is selected. This is because SOR is designed so that $\omega$ minimises the spectral radius $\rho(T_{SOR})$. 
\[\omega_{opt} = \frac{2}{1+\sqrt{1-\rho(T_{J})^{2}}}.\]
$\rho(T_{J})$ is the maximum absolute value from the eigenvalues of the matrix T (the matrix in front of x$^{k}$) used in the Jacobi method.
\[\rho(T_{J}) = \text{max}(\text{abs}(T_J)).\]
The SOR Method is: 
